...
func addActorOutboxTest(items vocab.ItemCollection, actor vocab.Item, ff ...filters.Check) tests.HTTPTest {
	...
	paginatedIRI := filterIRI(vocab.Outbox.IRI(actor), ff...)
	return tests.HTTPTest{
		Name: testName,
		Req:  tests.Request().IRI(paginatedIRI),
		Res: tests.Response().HasCode(http.StatusOK).
			ItemMatch(
				tests.HasID(paginatedIRI),
...
func addRootInboxTest(items vocab.ItemCollection, maxItems int, ff ...filters.Check) tests.HTTPTest {
	...
	paginatedIRI := filterIRI(vocab.Inbox.IRI(c2sRootIRI), ff...)
	return tests.HTTPTest{
		Name: testName,
		Req:  tests.Request().IRI(paginatedIRI),
		Res: tests.Response().HasCode(http.StatusOK).
	...
	for _, it := range objects {
		...
		toRun = append(toRun,
			tests.HTTPTest{
				Name: string(it.GetLink()),
				Req:  tests.Request().IRI(it.GetLink()),
				Res: tests.Response().HasCode(status).
					ItemMatch(
						tests.HasID(it.GetLink()),
						tests.IsType(it.GetType()),
					),
